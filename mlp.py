...
class LightNeuMF(torch.nn.Module):
    def __init__(self, config):
        super(LightNeuMF, self).__init__()
        self.config = config
...
class LightNeuMfEngine(Engine):
    """Engine for training & evaluating GMF model"""
    def __init__(self, config):
        self.model = LightNeuMF(config)
        if config['use_cuda'] is True:
            use_cuda(True, config['device_id'])
            self.model.cuda()
        super(LightNeuMfEngine, self).__init__(config)
        print(self.model)
